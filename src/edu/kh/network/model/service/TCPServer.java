...
public class TCPServer {
	...
	// ---->서버 소켓과 클라이언트 소켓이 연결되어 데이터 동신이 가능해짐
	
	public void serverStart() {
		
		// 1. 서버의 포트번호 정함 (0~65535까지 지정이 가능하지만 1023이하는 이미사용중인 경우가 많음)
		int port = 8500;
		...
		InputStream is = null; // is(Input Stream) 클라이언트 --> 서버 입력용 스트림변수
		BufferedReader br = null; // 입력용 보조 스트림변수
		
		OutputStream os = null;	// 서버--> 클라이언트로 출력하는 스트림변수 필요
		PrintWriter pw = null;  // 출력용 보조 스트림변수
		//BufferedWriter br = null;
		//PrintWriter에는 println, print, printf 등 다영한 출력함수를 제공하며 파일 출력을 편하게함
		
		try {
			// 2. 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port); // 서버용 소켓을 생성하여 포트결합
		
			
			// 3. 클라이언트 쪽에서 접속 요청이 오길 기다림
			// - 서버용 소켓은 생성되면 클라이언트 요청이 오기전까지 다음코들르 수행하지 않고 대기하고 있음
			System.out.println("[Server] ");
			System.out.println("클라이언트 요청을 기다리고 있습니다. . . ");
			
			// 4  접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓 객체 생성
			// --> 아래와 같이 요청을 수락하면 자동으로 Socket 객체가 얻어와
			...
			// close()도 밀어내는게 가능한데 굳이 flush()를 사용하는 이유는?
			//-->스트림을 닫지 않은 상태에서 내용을 내보내고 싶은 경우가 있음
			
			// 7-2) 클라이언트 --> 서버에게 입력(메세지 전송 받기)
			String message =br.readLine(); //클라이언트 메세지를 한줄 읽어옴
